...
def runq4():
  ...
  if wins >= nGames / 2:
    ...
    varianceWinScore = sum((score - averageWinScore) ** 2 for score in scores) / len(scores)
  print(('Average score of winning games: %d \n' % averageWinScore))
  print(('Score variance of winning games: %d \n' % varianceWinScore))
  return timeouts, wins, averageWinScore, varianceWinScore
...
class Test_4a(TestCase_A4):
  ...
  def test_0(self, set_leaderboard_value):
    ...
    timeouts, wins, averageWinScore, varianceWinScore = runq4()
    ...
    extra_credit = min((averageWinScore - 1300) // 100, 5)
    
    self.assertGreater(extra_credit, 0, msg=f'A valid extra credit should be higher than 0. Extra credit received: {extra_credit}')

    self.assertGreater(varianceWinScore, 0, msg=f'A valid solution should have some variance. Variance of the win scores: {varianceWinScore}')
