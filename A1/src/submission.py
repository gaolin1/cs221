...
def normalized_inner_products(A: np.ndarray, C: np.ndarray, normalize: bool = True) -> np.ndarray:
    ...
    if normalize:
        a_inner_c_normalized = a_inner_c/np.sqrt(A.shape[-1])
    else:
        a_inner_c_normalized = a_inner_c
    return a_inner_c_normalized
    # ### END CODE HERE ###
...
def matrix_grad(A: np.ndarray, B: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # ### START CODE HERE ###
    grad_A = einsum(B, 'k j -> k')
    grad_A = np.broadcast_to(grad_A,(A.shape[0],A.shape[1]))
    grad_B = einsum(A, 'i k -> k')
    grad_B = grad_B[:,None]
    grad_B = np.broadcast_to(grad_B, (B.shape[0],B.shape[1]))
...
def lsq_grad(w: np.ndarray, A: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    # ### START CODE HERE ###
    prediction = einsum(A, w, "n d, d -> n")
    residuals = prediction - b
    return einsum(A, residuals, "n d, n -> d")
    # ### END CODE HERE ###
...
def lsq_finite_diff_grad(w: np.ndarray,
    ...
    # ### START CODE HERE ###
    eye_vector = np.eye(w.shape[0])
    difference = eye_vector * epsilon
    forward_matrix = w + difference
    forward_prediction = einsum(A, forward_matrix, "n d, k d -> k n")
    forward_difference = 1/2*np.square(forward_prediction - b)
    noramlized_forward_difference = einsum(forward_difference, "d n -> d")
    backward_matrix = w - difference
    backward_prediction = einsum(A, backward_matrix, "n d, k d -> k n")
    backward_difference = 1/2*np.square(backward_prediction - b)
    noramlized_backward_difference = einsum(backward_difference, "d n -> d")
    return (noramlized_forward_difference-noramlized_backward_difference)/(2*epsilon)
    # ### END CODE HERE ###
